...
const createBooking = async (userId, bookingData) => {
  const { services: serviceItems, partnerId, address, scheduledAt, couponCode, offerId, packageId, notes } = bookingData;
  ...
  if (packageId) {
    const pkg = await packagesService.validateForBooking(packageId);
    baseAmount = parseFloat(pkg.price);
...
const respondServiceUpdate = async (userId, bookingId, action) => {
  ...
  if (action === "approve" && pending?.services) {
    ...
    const newTotal = parseFloat(pending.totalAmount || 0);
    ...
    await booking.update({
      ...
      partnerEarning: newTotal,
      serviceUpdatePending: false,
      pendingServicesUpdate: null,
    });
  } else {
    await booking.update({ serviceUpdatePending: false, pendingServicesUpdate: null });
  }
